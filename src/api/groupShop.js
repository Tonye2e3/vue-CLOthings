...
// 取得單一商品詳情
export const getGroupProduct = (id) => http.get(`/GroupProduct/${id}`).then((res) => res.data)

// ---- 購物車 ----
// 取得「目前登入者」的購物車內容：UserId 後端從 JWT 取得，前端不用也不能指定
export const getGroupCart = () => http.get('/GroupCart').then((res) => res.data)

// 加入購物車（同商品已存在的話，後端會自動累加數量）
export const addToGroupCart = (payload) => http.post('/GroupCart', payload).then((res) => res.data)
...
// 從購物車移除單一品項
export const removeGroupCartItem = (groupCartId) => http.delete(`/GroupCart/${groupCartId}`)

// 清空「目前登入者」的購物車
export const clearGroupCart = () => http.delete('/GroupCart/me')
...
export const getGroupOrders = () => http.get('/GroupOrder/mine').then((res) => res.data)
...
// 取消訂單
export const cancelGroupOrder = (orderId) => http.put(`/GroupOrder/${orderId}/cancel`)
